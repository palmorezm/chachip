...
shinyServer(function(input, output, session) {
    ...
    # Profile Box Outputs
    output$ova <- renderValueBox({
        title_ova <- "Good"
        valueBox(title_ova, subtitle = "Current Community Level",
                 icon = icon("credit-card"),
                 color = "green")
    }) 
    
    output$ovb <- renderValueBox({
        title_ovb <- "CAUTION"
        valueBox(title_ovb, subtitle = "Number of Cases per 100,000",
                 icon = icon("cloud-bolt-sun"),
                 color = "yellow")
    }) 
    
    output$ovc <- renderValueBox({
        title_ovc <- "DANGER"
        valueBox(title_ovc, subtitle = "Current Transmission Rate",
                 icon = icon("credit-card"),
                 color = "orange")
    })
